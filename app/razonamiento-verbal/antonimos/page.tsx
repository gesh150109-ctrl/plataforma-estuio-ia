...
import { diccionarioAntonimos } from "../../lib/antonimos"
...
export default function AntonimosPage() {
  ...
  const generarEjercicios = () => {
    const palabras = Object.keys(diccionarioAntonimos)
    ...
    const ejercicios: Ejercicio[] = seleccionadas.map(palabra => {
      const antonimos = diccionarioAntonimos[palabra]

      const correcta = antonimos[0]
      ...
      const extrasPremisa = antonimos.filter(a => a !== correcta)

      const extrasOpciones = opciones.map(op => {
        const entrada = Object.values(diccionarioAntonimos).find(lista =>
          lista.includes(op)
        )
        return entrada ? entrada.filter(a => a !== op).slice(0, 4) : []
      })
  ...
  return (
    <main className="min-h-screen bg-[#0a1124] text-white p-8 max-w-4xl mx-auto">
      ...
      <h1 className="text-3xl font-bold mb-6">
        📕 Antónimos — Nivel preuniversitario
      </h1>
        ...
        <div key={i} className="mb-6 bg-[#111c3a] p-6 rounded-2xl">
          ...
          {resultado !== null && (
            <p className="text-sm text-green-300 mb-3 ml-2">
              Antónimos: {p.extrasPremisa.join(", ")}
            </p>
          )}
