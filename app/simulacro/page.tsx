...
export default function SimulacroPage() {
  ...
  const iniciarSimulacro = async () => {
    ...
    const contenidoTotal = cursos
      .flatMap(c => c.temas.map(t => t.contenido))
      .join("\n\n")
    ...
    const res = await fetch("/api/ia", {
      ...
      body: JSON.stringify({
        ...
        cantidad: 40, // 👈 simulacro más exigente
      }),
    })
    ...
    const preguntasIA: Pregunta[] = json.resultado
      ...
      .map((bloque: string): Pregunta | null => {
        ...
        if (lineas.length < 6) return null
  ...
  if (!iniciado) {
    return (
      <main className="min-h-screen bg-[#0a1124] text-white p-8 text-center">
        <h1 className="text-3xl font-bold mb-6">🧠 Simulacro General</h1>
        ...
        >
          {cargando ? "Generando simulacro..." : "Iniciar simulacro"}
        </button>
      </main>
    )
  }
  ...
  if (resultado !== null) {
    return (
      <main className="min-h-screen bg-[#0a1124] text-white p-8 max-w-4xl mx-auto">
        <h1 className="text-3xl font-bold mb-4">📊 Resultado del simulacro</h1>
        <p className="text-xl mb-6">Nota final: {resultado}%</p>
  ...
  return (
    <main className="min-h-screen bg-[#0a1124] text-white p-8 max-w-4xl mx-auto">
      <div className="flex justify-between mb-6">
        <h1 className="text-xl font-bold">Simulacro en curso</h1>
